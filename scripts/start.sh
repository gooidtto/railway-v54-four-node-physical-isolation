...
# Optional Cloudflare node: pass variables through untouched so generate.py makes
# the same enable/disable decision that the supervisor makes below.
...
if len(lines) not in (3,4):raise SystemExit(f"FATAL: expected 3 or 4 nodes, got {len(lines)}")
...
try:
    urllib.request.urlopen(sys.argv[1],timeout=2).read();raise SystemExit(0)
...
CF_ENABLED=0
if [ -n "${CLOUDFLARE_TUNNEL_TOKEN:-}" ]&&[ -n "${CLOUDFLARE_PUBLIC_HOSTNAME:-}" ]&&[ -n "${CLOUDFLARE_ORIGIN_SERVICE:-}" ]&&[ -n "${WS_PORT:-}" ]&&[ -n "${WS_PATH:-}" ];then CF_ENABLED=1;fi
...
if [ "$CF_ENABLED" = 1 ];then
  ...
  cloudflared --no-autoupdate tunnel --metrics 127.0.0.1:2000 run --token "$CLOUDFLARE_TUNNEL_TOKEN" >"$D/cloudflared.log" 2>&1 & CFP=$!
  sleep 1
