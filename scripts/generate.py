...
# V54 SINGLE-8080 ROUTER
# Every Railway public entry targets application port 8080.
# 443 Domain -> 8080 -> HTTP -> 10086 XHTTP
...
def endpoint(env_domain, env_port, fallback_domain, fallback_port):
    h=(os.environ.get(env_domain) or fallback_domain).strip()
    p=(os.environ.get(env_port) or fallback_port).strip()
    try:p=int(p)
    except ValueError:raise SystemExit(f"FATAL: invalid {env_port}={p!r}")
