...
lines=[
 ...
 link(h,p,{'encryption':'none','security':'reality','sni':GRPC_SNI,'fp':FP,'alpn':'h2','pbk':PUBLIC_KEY,'sid':ids[3],'type':'grpc','serviceName':GRPC_SERVICE,'mode':'gun'},'VLESS gRPC REALITY 04 · TCP Proxy')]
if len(lines)!=NODE_COUNT:raise SystemExit(f'FATAL: expected {NODE_COUNT} nodes, got {len(lines)}')
state={'schema':17,'build':'test-4node-single-8080-grpc-reality-h2','architecture':'single-8080-sni-router','node_count':4,'application_port':8080,'tcp_proxy':{'domain':h,'port':p,'application_port':8080},'routes':{'raw1':{'sni':RAW1_SNI,'port':10087,'short_id':ids[0]},'xhttp':{'sni':XHTTP_SNI,'port':10088,'short_id':ids[1]},'raw2':{'sni':RAW2_SNI,'port':10089,'short_id':ids[2]},'grpc':{'sni':GRPC_SNI,'port':10090,'short_id':ids[3],'service_name':GRPC_SERVICE,'alpn':'h2'}},'xhttp_path':XPATH}
...
(D/'manifest.json').write_text(json.dumps({'schema':17,'build':'test-4node-single-8080-grpc-reality-h2','node_count':4,'application_port':8080,'distribution':{'raw1':'raw-reality-vision','xhttp':'xhttp-reality','raw2':'raw-reality-vision','grpc':'grpc-reality-h2'},'state_fingerprint':state['fingerprint']},indent=2)+'\n')
print('RELEASE=test-4node-single-8080-grpc-reality-h2',flush=True);print('SUBSCRIPTION_INVARIANT=4',flush=True);print(f'TCP {h}:{p} -> 8080 -> {RAW1_SNI} -> 10087 RAW REALITY Vision',flush=True);print(f'TCP {h}:{p} -> 8080 -> {XHTTP_SNI} -> 10088 XHTTP REALITY',flush=True);print(f'TCP {h}:{p} -> 8080 -> {RAW2_SNI} -> 10089 RAW REALITY Vision',flush=True);print(f'TCP {h}:{p} -> 8080 -> {GRPC_SNI} -> 10090 gRPC REALITY h2',flush=True);print('NODES=4',flush=True)
