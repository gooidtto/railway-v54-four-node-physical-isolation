...
def subscription(token):
    ...
    forbidden=("flow=", "host=", "pbk=", "sid=", "spx=", "fragment=", "os=")
    if any(any(f in x for f in forbidden) and "allowInsecure=1" not in x for x in lines): return None,"SUB_INVALID"
...
async def handle(reader,writer):
    ...
    async with SEM:
        try:
            ...
            if port==8080:
                if initial.startswith(HTTP): await http(reader,writer,initial)
                else: await relay(reader,writer,initial,DEST[8080],"8080-xhttp")
            elif port==8081:
                await relay(reader,writer,initial,DEST[8081],"8081-raw-reality")
            elif port==8082:
                await relay(reader,writer,initial,DEST[8082],"8082-grpc-reality")
            elif port==8083:
                await relay(reader,writer,initial,DEST[8083],"8083-ws-tls")
