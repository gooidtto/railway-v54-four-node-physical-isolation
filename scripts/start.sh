...
PUBLIC_DOMAIN="${RAILWAY_PUBLIC_DOMAIN:-}"; [ -n "$PUBLIC_DOMAIN" ] || { echo "ERROR: RAILWAY_PUBLIC_DOMAIN unavailable" >&2; exit 1; }
...
if [ ! -s "$WS_CERT" ] || [ ! -s "$WS_KEY" ] || ! grep -q "CN = $WS_HOST" "$D/ws_tls_cert_meta" 2>/dev/null; then
  ...
  printf '%s\n' "$WS_HOST" > "$D/ws_tls_cert_meta"; chmod 600 "$WS_CERT" "$WS_KEY"
fi
...
echo "BUILD=$BUILD_ID";echo "RAILWAY_PUBLIC_DOMAIN=$PUBLIC_DOMAIN";echo "8080 -> 10086 VLESS XHTTP TLS";echo "8081 -> 10087 VLESS RAW REALITY Vision";echo "8082 -> 10088 VLESS gRPC REALITY";echo "8083 -> 10089 VLESS WS TLS";echo "TCP1=${TCP_HOST}:${TCP_PORT}";echo "TCP2=${TCP2_HOST}:${TCP2_PORT}";echo "TCP3=${TCP3_HOST}:${TCP3_PORT}";echo "SUBSCRIPTION=https://${PUBLIC_DOMAIN}/sub/${TOKEN}";echo "NODES=4"
