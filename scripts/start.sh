...
BUILD_ID="fixed-8-node-baseline"
...
[ "$TCP_APP" = "8080" ] || { echo "fixed baseline requires Railway TCP Proxy target 8080; got ${TCP_APP:-unset}" >&2; exit 1; }
...
echo "BUILD=$BUILD_ID"
echo "TOPOLOGY=8080/gateway 10086/xhttp-tls 10087/reality-7-sni"
echo "TCP_PROXY=${TCP_HOST}:${TCP_PORT} -> 8080"
echo "REALITY_SNI_COUNT=7"
echo "SUBSCRIPTION=https://${PUBLIC_DOMAIN}/sub/${TOKEN}"
echo "NODES=8 (1 HTTPS XHTTP + 7 REALITY Vision SNI)"
echo "READY: build=$BUILD_ID gateway=8080 xray_reality=10087 xray_xhttp=10086 tcp_proxy=${TCP_HOST}:${TCP_PORT}"
