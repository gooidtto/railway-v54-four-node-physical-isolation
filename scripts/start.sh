...
set -eu
umask 077
BUILD_ID="stable-2node-single-8080"
...
GATEWAY_PORT=8080 REALITY_RAW_SNI="${REALITY_RAW_SNI:-www.cloudflare.com}" REALITY_RAW_TARGET="${REALITY_RAW_TARGET:-www.cloudflare.com:443}" \
...
if len(lines)!=2:raise SystemExit(f"FATAL: expected exactly 2 nodes, got {len(lines)}")
...
wait_port 127.0.0.1 10086 xhttp-http;wait_port 127.0.0.1 10087 xhttp-reality
...
echo "RELEASE=$BUILD_ID";echo "ARCHITECTURE=single-8080-router";echo "SUBSCRIPTION_INVARIANT=2";echo "TARGET_PORT=8080";echo "TCP=$TCP_HOST:$TCP_PORT -> 8080";echo "NODES=2"
