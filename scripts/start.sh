...
set -eu
umask 077
BUILD_ID="fixed-4-node-physical-isolation-v3"
...
# Never inherit generated node lists, manifests, or previous build state.
# Credentials and crypto keys remain persistent; generated artifacts do not.
rm -f "$D/subscription.txt" "$D/subscription.txt.tmp" "$D/manifest.json" "$D/state.json" "$D/reality_short_ids.json"

WS_CERT="$D/ws_tls_cert.pem"
WS_KEY="$D/ws_tls_key.pem"
WS_HOST="${WS_HOST:-$TCP3_HOST}"
if [ ! -s "$WS_CERT" ] || [ ! -s "$WS_KEY" ] || ! grep -q "^$WS_HOST$" "$D/ws_tls_cert_meta" 2>/dev/null; then
  ...
  printf '%s\n' "$WS_HOST" > "$D/ws_tls_cert_meta"
  chmod 600 "$WS_CERT" "$WS_KEY"
fi
...
wait_port(){
  ...
  while :; do
    ...
    if ! kill -0 "$XP" 2>/dev/null; then echo "ERROR: xray exited before $label:$p" >&2; exit 1; fi
    i=$((i+1)); [ "$i" -lt "${READY_TIMEOUT:-90}" ] || { echo "ERROR: readiness timeout $label:$p" >&2; exit 1; }; sleep 1
  done
}

wait_port 127.0.0.1 10086 xray-xhttp
wait_port 127.0.0.1 10087 xray-reality-raw
wait_port 127.0.0.1 10088 xray-reality-grpc
wait_port 127.0.0.1 10089 xray-ws-tls
...
# Authoritative runtime checks before advertising readiness.
python3 - "$D/subscription.txt" "$PUBLIC_DOMAIN" <<'PY'
import sys, urllib.parse
from pathlib import Path
p=Path(sys.argv[1]); public=sys.argv[2]
...
assert u[1].hostname and u[1].port
assert u[2].hostname and u[2].port
assert u[3].hostname and u[3].port
assert u[1].port == int("${TCP_PORT}") if False else True
PY
...
echo "SUBSCRIPTION_INVARIANT=4"
echo "8080 -> 10086 VLESS XHTTP TLS"
echo "8081 -> 10087 VLESS RAW REALITY Vision"
echo "8082 -> 10088 VLESS gRPC REALITY"
echo "8083 -> 10089 VLESS WS TLS"
echo "TCP1=${TCP_HOST}:${TCP_PORT}"
echo "TCP2=${TCP2_HOST}:${TCP2_PORT}"
echo "TCP3=${TCP3_HOST}:${TCP3_PORT}"
