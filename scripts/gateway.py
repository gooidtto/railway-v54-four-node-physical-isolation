...
async def relay(reader,writer,initial,dest,label):
    up=None;tasks=set()
    try:
        ur,up=await asyncio.open_connection(*dest);up.write(initial);await up.drain();print(f"[gateway] ROUTE={label} port={writer.get_extra_info('sockname')[1]} target={dest[0]}:{dest[1]}",flush=True)
...
async def main():
    ...
    servers=[await asyncio.start_server(handle,"0.0.0.0",p,limit=65536) for p in PORTS];print("GATEWAY_READY=8080->10086 8081->10087 8082->10088 8083->10089",flush=True)
