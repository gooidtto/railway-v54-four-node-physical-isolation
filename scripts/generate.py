...
# The running Railway service currently has three TCP proxies:
#   reseau:23337      -> 8081
#   interchange:23389 -> 8082
#   altaria:17903     -> 8083
# Railway only exposes one proxy through the automatic RAILWAY_TCP_* variables,
# so the other two are explicit environment variables. Defaults match the
# currently deployed Networking settings and can be overridden if Railway
# regenerates an external port.
...
# Seven fixed REALITY nodes are deliberately distributed across the three
# currently configured Railway TCP proxies: 3 + 2 + 2. This makes the
# subscription match the actual Networking page instead of silently putting
# all seven nodes on only the first proxy.
for i, sni in enumerate(snis, 1):
    ...
    lines.append(vless(proxy["domain"], proxy["port"], {
        ...
        "sni": sni, "fp": fp, "pbk": PUBLIC_KEY, "sid": short_id, "type": "tcp"
    }, f"VLESS REALITY Vision {i:02d} · {sni} · TCP {proxy['application_port']}"))
...
print("BUILD=fixed-8-node-three-tcp", flush=True)
...
print(f"REALITY=127.0.0.1:{REALITY_PORT} SNI_COUNT=7 DISTRIBUTION=3,2,2", flush=True)
