#!/usr/bin/env python3
import asyncio,base64,os,re,struct,urllib.parse,json
...
SEM=asyncio.Semaphore(int(os.environ.get('GATEWAY_MAX_CONNECTIONS','512')))
...
async def relay(reader,writer,initial,dest,label,sni='-'):
    ...
    try:
        ...
        print(f'[gateway] ROUTE={label} sni={sni}',flush=True)
        ...
        print(f'[gateway] RELAY_ERROR={label}:{type(e).__name__}:{e}',flush=True)
    finally:
        for t in tasks:
...
async def http(reader,writer,initial):
    ...
    parts=first.split(' ',2);method=parts[0] if parts else '';target=parts[1] if len(parts)>1 else ''
...
async def handle(reader,writer):
    async with SEM:
        try:
            ...
            print(f'[gateway] ERROR={type(e).__name__}:{e}',flush=True)
        finally:
...
async def main():
    ...
    print(f'GATEWAY_READY={PORT}',flush=True)
    print('[gateway] ROUTES='+','.join(f'{k}->{v[1]}' for k,v in ROUTES.items()),flush=True)
    print(f'[gateway] SUBSCRIPTION_EXPECTED={expected_nodes()}',flush=True)
