...
[ -n "$PUBLIC_DOMAIN" ] || {
  echo "FATAL: RAILWAY_PUBLIC_DOMAIN unavailable" >&2
  exit 1
}

TCP_HOST="${RAILWAY_TCP_PROXY_DOMAIN:-}"
TCP_PORT="${RAILWAY_TCP_PROXY_PORT:-}"
[ -n "$TCP_HOST" ] && [ -n "$TCP_PORT" ] || {
  echo "FATAL: one Railway TCP Proxy is required" >&2
  exit 1
}
...
if [ -s "$PRIV_FILE" ] && [ -s "$PUB_FILE" ]; then
  ...
  [ -n "$PRIVATE_KEY" ] && [ -n "$PUBLIC_KEY" ] || {
    echo "FATAL: failed to generate REALITY keys" >&2
    exit 1
  }
...
# ENV is used only once here to let generate.py validate and freeze runtime state.
# After generate.py completes, runtime.json is the single source of truth.
...
export CLOUDFLARE_TUNNEL_TOKEN="${CLOUDFLARE_TUNNEL_TOKEN:-}" \
  ...
  CLOUDFLARE_ORIGIN_SERVICE="${CLOUDFLARE_ORIGIN_SERVICE:-}" \
  WS_PORT="${WS_PORT:-}" \
  WS_PATH="${WS_PATH:-}"
...
[ -s "$RUNTIME" ] || {
  echo "FATAL: runtime state was not generated" >&2
  exit 1
}

# Read all routing decisions from the frozen runtime state.
CF_ENABLED=$(python3 - "$RUNTIME" <<'PY'
import json,sys
s=json.load(open(sys.argv[1]))
print("1" if s.get("cloudflare",{}).get("enabled") is True else "0")
PY
)
CF_HOST=$(python3 - "$RUNTIME" <<'PY'
import json,sys
print(json.load(open(sys.argv[1])).get("cloudflare",{}).get("public_hostname", ""))
PY
)
CF_ORIGIN=$(python3 - "$RUNTIME" <<'PY'
import json,sys
print(json.load(open(sys.argv[1])).get("cloudflare",{}).get("origin_service", ""))
PY
)
CF_PORT=$(python3 - "$RUNTIME" <<'PY'
import json,sys
v=json.load(open(sys.argv[1])).get("cloudflare",{}).get("ws_port")
print(v if v is not None else "")
PY
)
CF_PATH=$(python3 - "$RUNTIME" <<'PY'
import json,sys
...
if [ "$CF_ENABLED" = 1 ]; then
  [ -n "$CF_HOST" ] && [ -n "$CF_ORIGIN" ] && [ -n "$CF_PORT" ] && [ -n "$CF_PATH" ] || {
    echo "FATAL: runtime state says Cloudflare enabled but required state is incomplete" >&2
    exit 1
  }
  [ -n "${CLOUDFLARE_TUNNEL_TOKEN:-}" ] || {
    echo "FATAL: runtime state says Cloudflare enabled but tunnel token is unavailable" >&2
    exit 1
  }
fi
...
echo "CLOUDFLARE_HOST_STATE=$([ -n "$CF_HOST" ] && echo present || echo missing)"
echo "CLOUDFLARE_ORIGIN_STATE=$([ -n "$CF_ORIGIN" ] && echo present || echo missing)"
echo "CLOUDFLARE_PORT_STATE=$([ -n "$CF_PORT" ] && echo present || echo missing)"
echo "CLOUDFLARE_PATH_STATE=$([ -n "$CF_PATH" ] && echo present || echo missing)"
...
wait_port() {
  ...
  while :; do
    if python3 -c 'import socket,sys;s=socket.create_connection((sys.argv[1],int(sys.argv[2])),1);s.close()' "$h" "$p" 2>/dev/null; then
      echo "READY_CHECK=$label:$p"
      return 0
    fi
    if ! kill -0 "$XP" 2>/dev/null; then
      echo "FATAL: xray exited before $label:$p" >&2
      exit 1
    fi
    i=$((i+1))
    [ "$i" -lt "${READY_TIMEOUT:-90}" ] || {
      echo "FATAL: readiness timeout $label:$p" >&2
      exit 1
    }
    sleep 1
  done
...
PY
    then
      echo "READY_CHECK=$label"
      return 0
    fi
    i=$((i+1))
    [ "$i" -lt "${CLOUDFLARE_READY_TIMEOUT:-45}" ] || {
      echo "FATAL: readiness timeout $label" >&2
      exit 1
    }
    sleep 1
  done
...
if [ "$CF_ENABLED" = 1 ]; then
  wait_port 127.0.0.1 "$CF_PORT" cloudflare-ws-origin
fi
...
if [ "$CF_ENABLED" = 1 ]; then
  ...
  echo "CLOUDFLARE_WS_PORT=$CF_PORT"
  echo "CLOUDFLARE_WS_PATH=$CF_PATH"
  cloudflared --no-autoupdate tunnel --metrics 127.0.0.1:2000 run --token-file "$CF_TOKEN_FILE" >"$D/cloudflared.log" 2>&1 & CFP=$!
  sleep 1
  kill -0 "$CFP" 2>/dev/null || {
    echo "FATAL: cloudflared exited during startup" >&2
