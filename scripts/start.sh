...
TCP_HOST="${RAILWAY_TCP_PROXY_DOMAIN:-}"; TCP_PORT="${RAILWAY_TCP_PROXY_PORT:-}"; TCP_APP="${RAILWAY_TCP_APPLICATION_PORT:-}"
TCP2_HOST="${RAILWAY_TCP_PROXY_2_DOMAIN:-}"; TCP2_PORT="${RAILWAY_TCP_PROXY_2_PORT:-}"; TCP2_APP="${RAILWAY_TCP_PROXY_2_APPLICATION_PORT:-}"
TCP3_HOST="${RAILWAY_TCP_PROXY_3_DOMAIN:-}"; TCP3_PORT="${RAILWAY_TCP_PROXY_3_PORT:-}"; TCP3_APP="${RAILWAY_TCP_PROXY_3_APPLICATION_PORT:-}"
...
print(f"BUILD={sys.argv[2]}",flush=True); print("SUBSCRIPTION_INVARIANT=4",flush=True)
PY
...
u=[urllib.parse.urlsplit(x) for x in lines]; assert u[0].hostname==public and u[0].port==443
